...
export class SchoolManager {
  ...
  constructor() {
    ...
    if (fs.existsSync("./teachers.json")) {
      console.log("Listado de Docentes cargado en el sistema");
    } else {
  ...
  hireTeacher() {
    ...
    let numPhone = Number(
      readLineSync.question("Ingrese numero de telefono Profesor:-->")
    );
    let address = readLineSync
  ...
  expelStudent(
    ...
    let student=listStudent[deleteStudent]
    if (deleteStudent >= 0) {
      listStudent.splice(deleteStudent, 1);
      console.log(`El alumno ${student.name} ${student.lastName}, fue expulsado`);
    } else {
      console.log(`El alumno ${student.name} ${student.lastName}no fue encontrado en el listado`);
    }
  ...
  fireTeacher(
    ...
    let deleteTeacher = listTeacher.findIndex(
      ...
      let teacher=listTeacher[deleteTeacher]
    if (deleteTeacher >= 0) {
      listTeacher.splice(deleteTeacher, 1);
      console.log(`El docente ${teacher.name} ${teacher.lastName}fue despedido`);
    } else {
      console.log(`El docente ${teacher.name} ${teacher.lastName}no fue encontrado en el listado`);
    }
  ...
  editPerson(
    ...
    if (personStudent >= 0) {
      ...
      switch(dateEdit){
        ...
        break;
        case "e-mail":student.email=readLineSync.question("modifique el e-mail del alumno:-->").toLowerCase();
        break;
      }
     ...
     fs.writeFileSync("./students.json",JSON.stringify(listStudent));
      ...
      switch(dateEdit){
        ...
        break;
        case "e-mail":teacher.email=readLineSync.question("modifique el e-mail del alumno:-->").toLowerCase();
        break;
    ...
    listStudentByTeacher(id = Number(readLineSync.question("ingrese el DNI del Docente:-->"))) {
     ...
     averageStudents(){
      ...
      selectMenu(){
        ...
        let options = ["Inscribir Alumno", "Contratar Docente", "Expulsar Alumno","Despedir Docente","Editar Dato Alumno/Docente","Consultar Lista Alumno/Docente", "Consultar dato de Persona","Listado de Docente con sus Alumnos","Listado de Alumno con sus Docentes","Listado de Alumnos con Promedios"];
        ...
        while ((optionSelect+1) !== 0) {
            switch (option) {
                ...
                case "Consultar dato de Persona":
                    ...
                    this.consultPerson()
                    break;
                case "Listado de Docente con sus Alumnos":
                    console.log("Listado de Docente con sus Alumnos");
                    this.listStudentByTeacher()
                    break; 
                case "Listado de Alumno con sus Docentes":
                    console.log("listado de Alumno con sus docentes");
...
export let manager = new SchoolManager    
